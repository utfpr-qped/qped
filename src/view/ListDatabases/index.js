...
const ListDatabases = () => {
  ...
  const [shouldLoadDatabases, setShouldLoadDatabases] = useState(true)

  useEffect(() => {

    if (shouldLoadDatabases) {
      setShouldLoadDatabases(false)
      github.getQuestionsDatabasesNames().then(names => {
        setDatabases(names)
      })
    }

  }, [setDatabases, shouldLoadDatabases, setShouldLoadDatabases])

  function getRemote({ database }) {
    github.getQuestionsDatabase({ name: database }).then(content => {
      setParsedQuestions(content)
        ...
        databases.map((database, index) => {
          return (
            <div className="actions" key={index}>
              <div className="btn btn-outline-dark" onClick={() => { getRemote({ database }) }}>
                <span className="me-2">
